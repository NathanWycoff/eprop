...
for epoch in tqdm(range(EPOCHS)):
    with tf.GradientTape() as gt:
        ...
        for t in range(T_STEPS):
            ...
            #if len(spikes[t]) > 0:
            #    V += np.sum(tf.gather(THETA_rec, spikes[t], axis = 1), axis = 1)
            V += tf.matmul(THETA_rec, spikevec_last)

            # Integrate external stimuli
            #if len(in_spikes[t]) > 0:
            #    V += np.sum(tf.gather(THETA_in, in_spikes[t], axis = 1), axis = 1)
            ...
            #if len(spikes[t]) > 0:
            #    contrib = np.sum(tf.gather(THETA_out, spikes[t], axis = 1))
            #else:
            #    contrib = 0.
